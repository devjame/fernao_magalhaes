...
class BoxOpcao(MDGridLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.dialog = None
        self.opcao = [
            "Curioso para saber mais, dirijeste à mesa onde falam da grande viagem e é te proposto que te alistes nesta\
            demanda dirigida por Frenão de Magalhães, um capitão Português",
            "Continuas a beber a tua cerveja e no dia seguinte voltas à tua rotina de marinheiro miserável até ao fim \
            dos teus dias",
        ]
        for i in range(len(self.opcao)):
            self.lbl = MDLabel(text=self.opcao[i], font_size=14)
